var naukriFrameWork = function (e, a) {
    var b = this;
    if (!e) {
        return false
    }
    if (e == this && document.all) {
    ...
    if (e != document) {
        ...
        return new chainingFunction(b.el)
    } else {
        return new chainingFunction(e)
    }
};
...
(function () {
    ...
    d.matchesSelector = function (e,
        y) {
        return d(y, null, null, [e])
            .length > 0
    };
    d.find = function (E, e, F) {
        ...
        if (!D) {
            D = typeof e.getElementsByTagName !==
                "undefined" ? e.getElementsByTagName(
                    "*") : []
        }
        return {
            set: D,
    ...
    var b = d.getText = function (B) {
        ...
        if (e) {
            if (e === 1 || e === 9 ||
                e === 11) {
                if (typeof B.textContent ===
                    "string") {
                    return B.textContent
                } else {
                    if (typeof B.innerText ===
                        "string") {
                        return B.innerText
                            .replace(
                                u,
    ...
    var k = d.selectors = {
        ...
        find: {
            ...
            NAME: function (z, C) {
                if (typeof C.getElementsByName !==
                    "undefined") {
                    var y = [],
                        B = C.getElementsByName(
                            z[1]);
                    for (var A = 0,
                            e = B.length; A <
                        e; A++) {
            ...
            TAG: function (e, y) {
                if (typeof y.getElementsByTagName !==
                    "undefined") {
                    return y.getElementsByTagName(
                        e[1])
                }
            }
        },
        ...
        filters: {
            ...
            text: function (z) {
                var e = z.getAttribute(
                    "type"),
                    y = z.type;
                return z.nodeName.toLowerCase() ===
                    "input" &&
                    "text" === y &&
                    (e === y || e ===
                    null)
    ...
    for (var f in k.match) {
        k.match[f] = new RegExp(k.match[
            f].source + (
            /(?![^\[]*\])(?![^\(]*\))/
            .source));
        k.leftMatch[f] = new RegExp(
            /(^(?:.|\r|\n)*?)/.source +
            k.match[f].source.replace(
                /\\(\d+)/g, c))
    }
    k.match.globalPOS = j;
    var l = function (y, e) {
    ...
    if (document.documentElement.compareDocumentPosition) {
        p = function (y, e) {
            ...
            if (!y.compareDocumentPosition || !
                ...
                return y.compareDocumentPosition ? -
                    1 : 1
            }
            return y.compareDocumentPosition(
                e) & 4 ? -1 : 1
    ...
    (function () {
        ...
        k.order.splice(1, 0,
            "CLASS");
        k.find.CLASS = function (
            y, z, A) {
            if (typeof z.getElementsByClassName !==
                "undefined" && !
                A) {
                return z.getElementsByClassName(
                    y[1])
            }
        };
        e = null
...
$n.brewser = function () {
    ...
    var c = {
        ...
        safari: /webkit/.test(d) && !/chrome/
            .test(d),
        opera: /opera/.test(d),
        msie: /msie/.test(d) && !/opera/
            .test(d),
        mozilla: /mozilla/.test(d) && !/(compatible|webkit)/
            .test(d)
    };
    var b = null;
    var a = null;
...
var chainingFunction = function (el) {
    ...
    me.getStyle = function (obj,
        styleProp) {
        if (obj.currentStyle) {
            var y = obj.currentStyle[
                styleProp]
        } else {
            if (window.getComputedStyle) {
                var y = document.defaultView
                    .getComputedStyle(
                        obj, null).getPropertyValue(
                        styleProp)
            }
        }
        return y
    ...
    me.last = function () {
        if (el[0]) {
            ...
            return naukriFrameWork((
                    elem && elem.nodeType !=
                    1) ? elem.previousSibling :
                elem)
        }
        return me
    };
    me.toggletext = function (textOne,
        textTwo) {
        for (var i = 0; i < me.length; i++) {
            el[i].innerHTML == textOne ?
                el[i].innerHTML =
    ...
    me.replaceClass = function (classO,
        classN) {
        for (var i = 0; i < me.length; i++) {
            if (me.hasClass(el[i],
                classO)) {
    ...
    me.toggleClass = function (classO,
        classN) {
        for (var i = 0; i < me.length; i++) {
            if (classN) {
                if (me.hasClass(el[i],
    ...
    me.innerwidth = function () {
        ...
        if (document.compatMode ==
            ...
            innerW = document.documentElement
                .offsetWidth
        }
        if (window.innerWidth && window
            .innerHeight) {
    ...
    me.innerheight = function () {
        ...
        if (document.compatMode ==
            ...
            innerH = document.documentElement
                .offsetHeight
        }
        if (window.innerWidth && window
            .innerHeight) {
    ...
    me.clone = function () {
        for (var i = 0; i < me.length; i++) {
            var state = (typeof arguments[
                0] == "undefined") ?
                true : arguments[0];
            return el[0].cloneNode(
                state)
        }
    };
    me.append = function (child) {
        for (var i = 0; i < me.length; i++) {
            if ((typeof child).toLowerCase() ==
                "string") {
                el[i].innerHTML +=
                    child
            } else {
                el[i].appendChild(me.getObj(
                    child))
            }
        }
        return me
    ...
    me.prepend = function (child) {
        for (var i = 0; i < me.length; i++) {
            var newNode = me.getObj(
                child);
            var firstChild =
                naukriFrameWork(el[i]).first()
                .currObj();
				
				if ((typeof child).toLowerCase() =="string") {
                el[i].innerHTML = child+el[i].innerHTML;
            } else {
                el[i].insertBefore(newNode,
                firstChild)
            }
    ...
    me.each = function (callback, args,
        object) {
        if (!object) {
            object = this
        }
    ...
    me.appendTo = function (parent1) {
        for (var i = 0; i < me.length; i++) {
            me.getParentObj(parent1).appendChild(
                el[i])
        }
        return me
    };
    me.getParentObj = function (parent1) {
        if ((typeof parent1).toLowerCase() ==
            "string") {
            parent1 = naukriFrameWork(
                parent1).currObj()
        } else {
            if ((parent1.toString()).toLowerCase() ==
                "[object object]") {
    ...
    me.addEvent = function (eventName,
        fn) {
        for (var i = 0; i < me.length; i++) {
            window.attachEvent ? el[i].attachEvent(
                "on" + eventName, fn) :
    ...
    me.removeEvent = function (
        eventName, fn) {
        for (var i = 0; i < me.length; i++) {
            if (fn) {
                window.detachEvent ? el[
                    i].detachEvent("on" +
                    eventName, fn) : el[
    ...
    var currApp = (navigator.appName ==
        "Netscape") ? true : false;
    me.slideUp = function () {
        var sstyle = 0;
        for (var i = 0; i < me.length; i++) {
            var padT = parseInt(me.eq(i)
                .getStyle(me.eq(i).currObj(), (
                    currApp ?
                    "padding-top" :
                    "paddingTop")));
            padT += parseInt(me.eq(i).getStyle(
                me.eq(i).currObj(), (
                    currApp ?
                    "padding-bottom" :
                    "paddingBottom"
                )));
            var offH = (me.eq(i).height()) -
                (padT + 2);
            sstyle = offH;
            if (offH > 0) {
                var t = setInterval(
    ...
    me.removeAttr = function (a) {
        ...
        for (var i = 0; i < me.length; i++) {
            me.eq(i).currObj().removeAttribute(
                attr)
        }
    };
    me.ready = function (i) {
...
chainingFunction.prototype.ajaxReq =
    function (param) {
  var xhr = null;
  ...
  cache = (typeof cache == 'undefined' || cache) ? true : false;
  ...
  xhr.onreadystatechange = function(){
    ...
    else if(xhr.readyState == 4){error(xhr);return}
  };
if(timeout)
  xhr.timeout = (timeout.time)?timeout.time:3E4;
...
$n.trim = function (b, a) {
    return $n.ltrim($n.rtrim(b, a), a)
};
$n.ltrim = function (b, a) {
    a = a || "\\s";
    return b.replace(new RegExp("^[" +
        a + "]+", "g"), "")
};
$n.rtrim = function (b, a) {
    a = a || "\\s";
